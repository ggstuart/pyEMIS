...
#  if n > 0:
#    print "%s points removed from ends ([0:%s] to [%s:%s])" % (n, len(date)-1, start, end)
#  else:
#    print "No points removed from ends"
...
def rate(date, integ):
  ...
  if any(gap == 0): 
    a = (gap==0)
    raise zeroGapError, [time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(d)) for d in date[a]]
